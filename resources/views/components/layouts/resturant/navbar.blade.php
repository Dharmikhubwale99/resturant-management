<nav class="bg-white shadow-lg sticky top-0 z-50" x-data="{
    ...
    start: 0,
    visible: 5,
    openDropdown: null,        // desktop dropdown
    openMobileDropdown: null,  // NEW: mobile dropdown (for Android/Mobile)
    menuLinks: [
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16 items-center">
                        ...
                        <template x-for="(link, index) in menuLinks.slice(start, start + visible)" :key="index">
                            <div class="relative">
                                ...
                                <template x-if="!link.children">
                                    <a :href="link.href || '#'"
                                       :target="link.newTab ? '_blank' : null" x-text="link.text"
            ...
            <div class="flex items-center space-x-4">
                ...
                <!-- Profile Dropdown -->
                <div class="relative">
                    <button @click="profileMenuOpen = !profileMenuOpen; if(!profileMenuOpen){ openMobileDropdown=null }"
                    ...
                    <!-- Dropdown Menu -->
                    <div x-show="profileMenuOpen" @click.away="profileMenuOpen = false; openMobileDropdown=null"
                        ...
                        <div class="block md:hidden max-h-60 overflow-y-auto" @click.outside="openMobileDropdown = null">
                            <template x-for="(link, i) in menuLinks" :key="i">
                                <div class="last:border-0 border-gray-100">
                                    ...
                                    <!-- With children (e.g., Setting) -->
                                    <template x-if="link.children">
                                        <div class="px-2">
                                            <button type="button"
                                                    @click="openMobileDropdown === i ? openMobileDropdown = null : openMobileDropdown = i"
                                                    class="w-full flex items-center justify-between px-2 py-2 text-sm text-gray-700 hover:bg-gray-100">
                                                <span x-text="link.text"></span>
                                                <svg class="w-4 h-4 transition-transform"
                                                     :class="{'rotate-180': openMobileDropdown === i}" viewBox="0 0 20 20" fill="currentColor">
                                                    <path fill-rule="evenodd"
                                                          d="M5.23 7.21a.75.75 0 011.06.02L10 10.94l3.71-3.71a.75.75 0 111.06 1.06l-4.24 4.24a.75.75 0 01-1.06 0L5.21 8.29a.75.75 0 01.02-1.08z"
                                                          clip-rule="evenodd" />
                                                </svg>
                                            </button>
                                            <div x-show="openMobileDropdown === i" x-transition class="pl-4 pb-2">
                                                <template x-for="child in link.children" :key="child.text">
                                                    <a :href="child.href"
                                                       class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100"
                                                       x-text="child.text"></a>
