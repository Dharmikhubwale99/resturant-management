  ...
  <div class="modal fade" id="uploadModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
            ...
            <div class="form-group" id="attachment">
              <div class="controls text-center">
                <div class="input-group w-100">
                  ...
                  {{-- PLAN-LIMIT BADGE (just below the Choose File(s) button) --}}
                @php
                $restaurant = auth()->user()?->restaurants()?->first();
                $planKb     = (int) ($restaurant?->max_file_size_kb ?? 0);       // from restaurants.max_file_size_kb
                $lfmKb      = (int) $helper->maxUploadSize();                    // LFM’s own limit (KB)
                $effectiveKb = $planKb ? min($planKb, $lfmKb) : $lfmKb;          // enforce the stricter of the two
                $effectiveMb = $effectiveKb ? number_format($effectiveKb / 1024, 2, '.', '') : null; // Dropzone wants MB
                @endphp

                @if ($effectiveMb)
                <div class="text-center small text-muted mt-2">
                Max per file: {{ $effectiveMb }} MB
                </div> @endif
    ...
    <script>
       Dropzone.options.uploadForm = {
            ...
            @if ($effectiveMb) maxFilesize: {{ $effectiveMb }}, @endif

            init: function () {
                var dz = this;

                function isOk(res) {
                if (typeof res === 'string') return res.trim() === 'OK';
                return !!(res?.uploaded || res?.success || res?.status === 'success' || res?.message === 'OK');
                }

                this.on('success', function(file, response) {
                if (isOk(response)) {
                    // clear any accidental error UI if set
                ...
                // Only once when all files are done: refresh list + clear previews
                this.on('queuecomplete', function () {
                loadFolders();           // your LFM refresh function
                dz.removeAllFiles(true); // clean previews
                });

                // Human-readable error text (no [object Object])
                this.on('error', function(file, message, xhr) {
                if (xhr && xhr.responseText) message = xhr.responseText;
                if (typeof message === 'object') message = (message.message || message.error || JSON.stringify(message));
                alert(message);
                });
            }
        };
