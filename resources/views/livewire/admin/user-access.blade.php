        ...
        <div class="space-y-2">
            @foreach ($modules as $module)
                <div class="flex items-center space-x-3" wire:key="module-{{ $module->id }}">
                    <input type="checkbox" wire:model="access.{{ $module->id }}" @checked(data_get($access, $module->id, false))>
                    <label>{{ $module->key }}</label>
                </div>
            @endforeach
        </div>

        <button type="submit" class="mt-4 px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">Save
            Access</button>
    </form>
