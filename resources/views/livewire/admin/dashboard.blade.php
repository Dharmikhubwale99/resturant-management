<div class="p-4 sm:p-6">
    <h2 class="text-xl sm:text-2xl font-bold mb-4">Dashboard</h2>

    <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-6 mb-4 sm:mb-6">
        <div class="bg-white shadow rounded p-2 sm:p-4">
            <p class="text-gray-500 text-xs sm:text-sm">Today Resturant / Total Restaurants</p>
            <p class="text-lg sm:text-2xl font-semibold">{{ $todayRestaurants }} / {{ $totalRestaurants }}</p>
        </div>
        <div class="bg-white shadow rounded p-2 sm:p-4">
            <p class="text-gray-500 text-xs sm:text-sm">Today Free Trial / Total Free Trial</p>
            <p class="text-lg sm:text-2xl font-semibold">
                {{ $todayFreeTrialCount }} / {{ $freeTrialCount }}
            </p>
        </div>


        <div class="bg-white shadow rounded p-2 sm:p-4">
            <p class="text-gray-500 text-xs sm:text-sm">Today Sale / Total Sale </p>
            <p class="text-lg sm:text-2xl font-semibold"> &#8377; {{ number_format($todayincome, 2) }} / &#8377;
        ...
        @if (Auth::user()->hasRole('dealer'))
            <div class="bg-white shadow rounded p-2 sm:p-4">
                ...
                <p class="text-lg sm:text-2xl font-semibold">&#8377; {{ number_format($user->dealer_commission, 2) }}
                </p>
            </div>
        @endif
        ...
        @if (Auth::user()->hasRole('superadmin'))
            <div class="bg-white shadow rounded p-2 sm:p-4">
                <p class="text-gray-500 text-xs sm:text-sm">Today Dealer Sale / Total Dealer Sale</p>
                <p class="text-lg sm:text-2xl font-semibold">{{ $todayDealerSalesCount }} / {{ $totalDealerSalesCount }}</p>
            </div>
        @endif
        ...
        <div class="flex flex-col gap-3 sm:gap-4 mb-3 sm:mb-4">
            <div class="flex justify-between items-center">
                ...
                @if (Auth::user()->hasRole('superadmin'))
                    <button wire:click="export" wire:loading.attr="disabled"
                        class="bg-green-500 text-white px-2 py-1 sm:px-3 sm:py-1 rounded text-xs sm:text-sm flex items-center gap-1">
            ...
            <div class="grid grid-cols-1 sm:flex sm:flex-row gap-2">
                ...
                <select wire:model.live="filterPeriod"
                    class="border rounded px-2 py-1 sm:px-3 sm:py-1 text-xs sm:text-sm">
            ...
            @if ($showCustomRange)
                <div class="grid grid-cols-2 sm:flex sm:flex-row gap-2">
                    ...
                    <button wire:click="applyCustomRange"
                        class="col-span-2 sm:col-span-1 bg-blue-500 text-white px-2 py-1 rounded text-xs sm:text-sm">
                        Apply Dates
                    </button>
                </div>
            @endif
            ...
            <table class="min-w-full divide-y divide-gray-200 text-xs sm:text-sm">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-2 py-1 sm:px-4 sm:py-2 text-left font-medium text-gray-500 uppercase">Reffered By</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse ($purchases as $index => $resto)
                        <tr>
                            ...
                            <td class="px-2 py-1 sm:px-4 sm:py-2 whitespace-nowrap">{{ $resto->plan->name ?? '-' }}
                            </td>
                            <td class="px-2 py-1 sm:px-4 sm:py-2 whitespace-nowrap">
                                &#8377;{{ $resto->plan->price ?? 0 }}</td>
                            <td class="px-2 py-1 sm:px-4 sm:py-2 whitespace-nowrap">{{ $resto->mobile }}</td>
                            <td class="px-2 py-1 sm:px-4 sm:py-2 whitespace-nowrap">
                                {{ $resto->created_at ? \Carbon\Carbon::parse($resto->created_at)->format('d-m-Y') : '' }}
                            </td>
                            <td class="px-2 py-1 sm:px-4 sm:py-2 whitespace-nowrap">{{ $resto->user->referredBy->username }}</td>
                            <td class="px-2 py-1 sm:px-4 sm:py-2 whitespace-nowrap">
                                {{ $resto->plan_expiry_at ? \Carbon\Carbon::parse($resto->plan_expiry_at)->format('d-m-Y') : '' }}
                            </td>
                        </tr>
                    @empty
                        <tr>
