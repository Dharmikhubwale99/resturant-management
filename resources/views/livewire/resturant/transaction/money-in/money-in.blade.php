        ...
        <table class="min-w-full divide-y divide-gray-200 border border-gray-300">
            <thead class="bg-gray-100">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left text-sm font-semibold text-gray-700">Issue</th>
            ...
            <tbody class="divide-y divide-gray-200">
                @foreach ($logs as $payment)
                    ...
                    <tr class="hover:bg-gray-50">
                        <td class="px-6 py-3 text-sm text-gray-900">{{ $payment->id ?? '—' }}</td>
                        <td class="px-6 py-3 text-sm text-gray-900">{{ $payment->order->order_number ?? '—' }}</td>
                        ...
                        <td class="px-6 py-3 text-sm text-gray-900">{{ ucfirst($payment->method) }}</td>
                        <td class="px-6 py-3 text-sm text-gray-900">
                        ...
                        <td class="px-6 py-3 text-sm text-gray-900">{{ $firstLog?->issue ?? '—' }}</td>
                        <td class="px-6 py-3 text-sm text-gray-900">{{ $payment->payment_date ?? $firstLog?->created_at?->format('d-m-Y H:i') ?? '—' }}</td>
                        <td class="px-6 py-3 text-sm text-gray-900">
                            @if ($remaining > 0)
