<div class="min-h-screen flex items-center justify-center bg-gray-100">
    <div class="p-6 bg-white rounded shadow max-w-3xl w-full">
        ...
        <form wire:submit.prevent="submit" class="space-y-4">
            ...
            {{-- <x-form.select name="item_type" label="Item Type" wire:model="item_type" required>
                <option value="">Select item type</option>
                @foreach ($itemTypes as $type)
                    <option value="{{ $type->value }}">{{ $type->label() }}</option>
                @endforeach
            </x-form.select> --}}
            <x-form.select name="item_type"
               label="Item Type"
            ...
            <x-form.input name="short_name" label="Short Name" wireModel="short_name" 
