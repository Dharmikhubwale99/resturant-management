<div class="p-6 bg-white rounded shadow">
    ...
    <div class="flex justify-between items-center mb-4">
        <h2 class="text-xl font-bold">Table List</h2>
         <div class="flex items-center gap-4">
            ...
            />
            @can('table-create')
                <x-form.button title="+ Add" route="restaurant.tables.create" class="bg-blue-600 hover:bg-blue-700 text-white" />
            @endcan
        </div>
    </div>
    <x-form.error />
        ...
        <table class="min-w-full divide-y divide-gray-200 border border-gray-300">
            <thead class="bg-gray-100">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left text-sm font-semibold text-gray-700">QR Enabled</th>
                    <th class="px-6 py-3 text-left text-sm font-semibold text-gray-700">Status</th>
                ...
                <tbody class="divide-y divide-gray-200">
                    @foreach ($tables as $table)
                        <tr class="hover:bg-gray-50">
                            ...
                            @if (setting('area_module'))
                            <td class="px-6 text-sm text-gray-900">{{ $table->area->name ?? '' }}</td>
                            @endif
                            <td class="px-6 text-sm text-gray-900">{{ $table->name }}</td>
                            <td class="px-6 text-sm text-gray-900">{{ $table->capacity }}</td>
                            <td class="px-6 text-sm text-gray-900">{{ ucfirst($table->status) }}</td>
                            <td class="px-6 text-sm text-gray-900">{{ $table->qr_enabled ? 'Yes' : 'No' }}</td>
                            ...
                            <td class="px-2 text-sm text-gray-900">
                                ...
                                <x-form.button title=""
                                        ...
                                        wireClick="confirmBlock({{ $table->id }})">
                                        @if ($table->is_active)
                                ...
                                @can('table-show')
                                    <x-form.button title=""
                                        class="w-8 h-8 rounded flex items-center justify-center" :route="['restaurant.tables.show', $table->id]">
                                ...
                                @can('table-edit')
                                    <x-form.button title=""
                                        class="w-8 h-8 rounded flex items-center justify-center" :route="['restaurant.tables.edit', $table->id]">
                                        <span class="w-4 h-4">
                                ...
                                @can('table-delete')
                                    <x-form.button title=""
                                        ...
                                        wireClick="confirmDelete({{ $table->id }})">
                                        <span class="w-4 h-4">
            ...
            @if ($confirmingDelete)
                <div class="fixed inset-0 bg-transparent bg-opacity-0 z-40 flex items-center justify-center">
                    <div class="bg-white rounded-lg p-6 shadow-xl z-50 w-full max-w-md">
                        ...
                        <p class="text-gray-700 mb-6">Are you sure you want to delete this table? This action cannot be
                            undone.</p>
                ...
                <div class="fixed inset-0 bg-transparent bg-opacity-0 z-40 flex items-center justify-center">
                    <div class="bg-white rounded-lg p-6 shadow-xl z-50 w-full max-w-md">
                        <h3 class="text-lg font-semibold mb-4 text-yellow-600">
                            {{ optional(\App\Models\Table::find($tableId))->is_active ? 'Confirm Unblock' : 'Confirm Block' }}
                        </h3>
                        <p class="text-gray-700 mb-6">
                            Are you sure you want to
                            {{ optional(\App\Models\Table::find($tableId))->is_active ? 'unblock' : 'block' }}
                            this
                            Table?
                        </p>
