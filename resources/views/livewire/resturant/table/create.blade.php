<div class="min-h-screen flex items-center justify-center bg-gray-100">
    <div class="p-6 bg-white rounded shadow max-w-3xl w-full">
        <h2 class="text-2xl font-bold mb-6 text-center">Add Table</h2>
        <x-form.error />
        <form wire:submit.prevent="submit" class="space-y-4">
            @if (setting('area_module'))
            <x-form.select name="area_id" label="Area" wire:model="area_id" :options="$areas" />
            @endif
            <x-form.input name="name" label="Table Name" wire:model="name" required />
            <x-form.input name="capacity" label="Capacity" wire:model="capacity" type="number" />
